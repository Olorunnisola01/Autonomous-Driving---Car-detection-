...
def generate_missing_images():
    """Generate placeholder images for any missing scene steps."""
    count = 0
    for scene_id, _, _, _, act_num in SCENES:
        ...
        existing = [p for p in act_dir.iterdir() 
                    if p.is_file() and p.suffix.lower() in IMAGE_EXTENSIONS and p.name.startswith(f"{scene_id}_")]
        if len(existing) >= 10:
            continue
        ...
        for step_num, step_desc in enumerate(STEP_DESCRIPTIONS[scene_id], 1):
            ...
            if stem not in existing_stems:
                ...
                if not filepath.exists():
                    img = create_placeholder_image(scene_id, step_num)
...
def inline_thumbnail(path: Path, max_size: int = 520) -> tuple:
    """Return (base64_string, orig_w, orig_h) for a 16:9 contained thumbnail."""
    with Image.open(path) as raw:
        image = ImageOps.exif_transpose(raw).convert("RGB")
        original_w, original_h = image.size
        
        # 16:9 contain: create a 16:9 canvas and paste the image centered
        target_w = max_size
        target_h = int(max_size * 9 / 16)
        
        # Fit image within target maintaining aspect ratio (contain)
...
def build_character_refs_section(refs):
    cards_html = []
    for path in refs:
        try:
            data, orig_w, orig_h = inline_thumbnail(path)
            label = label_for(path)
            cards_html.append(
                f'<article class="card">'
                f'<img loading="lazy" src="data:image/jpeg;base64,{data}" alt="{html.escape(label)}">'
                f'<div class="meta"><h3>{html.escape(label)}</h3>'
                f'<p>{html.escape(path.relative_to(ROOT).as_posix())} · {orig_w}×{orig_h}</p></div></article>'
            )
        except Exception as e:
            print(f"  Warning: Error processing {path.name}: {e}")
    
    return f'''<section id="character-refs">
        <div class="section-heading">
            <h2>Character References</h2>
            <span>{len(refs)} characters</span>
        </div>
        <div class="grid">
            {"".join(cards_html)}
        </div>
...
def build_scene_section(scene_id, scene_title, voiceover, location, images):
    cards_html = []
    for path in images:
        try:
            data, orig_w, orig_h = inline_thumbnail(path)
    ...
    if not cards_html:
        cards_html.append('<p class="empty">No images generated yet.</p>')
    
    return f'''<section class="scene-section" id="{scene_id}">
        <div class="scene-header">
            <h3>{scene_id} — {html.escape(scene_title)}</h3>
            <div class="scene-location">Location: {html.escape(location)}</div>
            <div class="voiceover">{html.escape(voiceover)}</div>
        </div>
...
def build_act_section(act_num, act_name, scene_ids, act_images, scene_map):
    parts = [f'''<div class="act-header" id="act-{act_num}">
        <h2>Act {act_num} — {act_name}</h2>
        <div class="act-subtitle">Scenes {scene_ids[0]}–{scene_ids[-1]}</div>
    </div>''']
    
    for scene_id in scene_ids:
        info = scene_map[scene_id]
        scene_imgs = [p for p in act_images if p.name.startswith(f"{scene_id}_")]
        parts.append(build_scene_section(scene_id, info["title"], info["voiceover"], 
                                         info["location"], scene_imgs))
    
    return "\n".join(parts)


def build_gallery():
    """Build the complete storyboard gallery HTML."""
    print("=" * 60)
    print("THE DUKE'S OBSESSION — Gallery Builder")
    print("=" * 60)
    
    print("\n1. Generating missing placeholder images...")
    generate_missing_images()
    
    # Build scene lookup map
    scene_map = {}
    for sid, title, vo, loc, act in SCENES:
        scene_map[sid] = {"title": title, "voiceover": vo, "location": loc, "act": act}
    ...
    # Build sections
    sections = [build_character_refs_section(refs)]
    for act_num in range(1, 6):
        act_name, scene_ids = ACTS[act_num]
        sections.append(build_act_section(act_num, act_name, scene_ids, acts[act_num], scene_map))
    
    body = "\n".join(sections)
    
    # Navigation items for sidebar
    nav_items = ['<a href="#character-refs">Character References</a>']
    for act_num in range(1, 6):
        act_name, scene_ids = ACTS[act_num]
        nav_items.append(f'<a href="#act-{act_num}" class="act-link">Act {act_num} — {act_name}</a>')
        for sid in scene_ids:
            nav_items.append(f'<a href="#{sid}" class="scene-link">{sid} — {scene_map[sid]["title"]}</a>')
...
header {{
    ...
    text-align: center;
    background: linear-gradient(180deg, #1a0a0f 0%, #0a0a0f 100%);
...
header h1 {{
    ...
    margin-bottom: 1rem;
    text-shadow: 0 2px 30px rgba(201,169,97,0.2);
}}
header .subtitle {{
    font-size: 1.1rem;
...
.scene-header {{
    ...
    padding: 1.5rem 2rem;
    background: linear-gradient(135deg, rgba(139,26,26,0.08) 0%, rgba(10,10,15,0.9) 100%);
